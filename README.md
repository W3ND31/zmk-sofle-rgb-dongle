# Sofle V2 RGB - ZMK Configuration
...
- **Display**: No display (?)
- **RGB Underglow**: WS2812 LEDs (36 LEDs per side, per-key RGB with underglow support) [DISABLED]
...
- `sofle_left.uf2` - Firmware for the left side
- `sofle_right.uf2` - Firmware for the right side
...
sofle-v2-oled-zmk/
├── config/
│   ├── sofle.conf       # Global configurations
│   └── sofle.keymap     # Main keymap
